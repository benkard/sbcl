...
(defun prepare-lambda (body name current-path source-location)
  ...
  (let ((body* (prepare-progn body)))
    (eval-lambda (%lambda current-path source-location)
      (let ((env *env*)
  ...
  (values
   (cond
     ((self-evaluating-p form)
      (eval-lambda (quote) form))
     (t
      (etypecase form
        (symbol
         (ecase form
           ((%argnum)
            (eval-lambda (%argnum) *argnum*))))
        (cons
         (case (first form)
           ((%with-environment)
           ...
           ((multiple-value-call)
            (destructuring-bind (f &rest argforms) (rest form)
              (let ((f* (prepare-form f))
                ...
                (eval-lambda (multiple-value-call)
           ...
           ((multiple-value-prog1)
            (destructuring-bind (values-form &body body) (rest form)
              (let ((values-form* (prepare-form values-form))
                    (body*        (prepare-progn body)))
                (eval-lambda (multiple-value-prog1)
                  (multiple-value-prog1
                      (funcall values-form*)
                    (funcall body*))))))
           ((progn)
